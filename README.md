...
一个用于矫正照片透视畸变的 prototype。和 Snapseed 的"透视"功能定位类似，但**只做裁剪、不做填充**——输出图里的每一个像素都来自原图，不会出现填黑、填白、或 AI 生成的"假背景"。
...
| `--strength FLOAT` | `1.0` | 矫正强度，`0.0`=完全不动，`1.0`=完全矫正，中间值=线性插值 |
...
4. **构造单应矩阵**：让消失点跑到无穷远的方向（竖直 = 正上方，水平 = 正左/右）
5. **warp 图像**：应用矩阵，得到矫正后的图（此时画面边缘是个不规则四边形）
6. **找最大内接矩形**：在这个四边形里找最大的轴对齐矩形——这就是"只裁剪、不填充"的关键
7. **（可选）按原图比例再居中裁一刀**
...
- 焦距是用 `max(width, height)` 猜的，没读 EXIF 的真实焦距——绝大多数照片够用，极端广角/长焦镜头可能不准
...
- **HDR 不保留**：iPhone 的 HDR 照片在 HEIC 容器里是"主图 + gainmap 辅助层"。当前 pipeline 解码时只取主图，输出 JPEG 也没地方放 gainmap，所以保存后 iOS Photos 把它当 SDR 渲染。
  ...
  - Apple 自己的官方 HDR 编辑 API（`CIContext.writeHEIFRepresentation` + `kCGImageAuxiliaryDataTypeISOGainMap`）只在 Swift/Obj-C 里能调，Python web 路线没有等价物。要做"保留 HDR"，路径基本只剩 Apple 推荐的 iOS 原生 app + Photo Editing Extension。
...
- 同 WLAN 的 iPhone / iPad：替换成 Mac 的局域网 IP，例如 `http://192.168.x.x:8000/`
...
- **拖拽**：水平位移只调整相机偏航（yaw），垂直位移只调整俯仰（pitch）；单指拖拽不会主动加入绕光轴的滚转（roll）增量
- **按住"原图"按钮**：临时显示未校正源图，抬手回到校正后；和拖拽互斥
- **保存**：iOS 走 Web Share API 弹分享菜单（选"存储到照片"直接进相册）；桌面下载 JPG。EXIF（拍摄时间、相机、GPS、镜头）、ICC profile 全保留
...
4. 最终 warp = `S_crop · K · R · K⁻¹`，按构造不会引入任意剪切或非均匀缩放

手动拖拽从按下时姿态计算 `yaw = atan(dx/fx)`、`pitch = -atan(dy/fy)`，再在 SO(3) 上合成。两维手势只控制两维相机轴，不再为了让某个角点严格跟手而混入滚转；从画面任意位置开始的同向、同距离手势因此具有相同含义。

前端 `webapp/geometry.js` 和后端 `geometry.py` 是同一份几何合同；随机旋转 contract tests 会逐项比较两边的 matrix、crop 和角度。保存接口直接发送 correction state，旧 corners API 只作为兼容路径保留。
...
`window.__rejectStats` 实时统计每个阈值触发次数，控制台可读：用一阵子之后看分布能反过来 calibrate 这几个数字（某个常年不触发可删；某个触发频繁说明值太紧）。

### 算法的能力边界（这个工具对什么场景不工作）

当前的自动检测本质上是一个**线段对齐器**：找直线段 → 按角度聚类 → RANSAC 找消失点 → 构造矫正矩阵。这个模型的隐含假设是 **"画面里的主要线段方向 = 世界的正交轴"**。这个假设在下面的场景里成立得很差，按"出错的危险程度"排：

- **建筑、文档、屏幕、招牌、桌面**（人造强直线 + 正交结构）：✅ 算法的甜区，效果接近最优
- **金字塔 / 帐篷 / 圆顶**（无竖线，但人眼能感知"指向天的方向"）：算法看不到"隐含的竖直"，要么完全失败（VP 找不到），要么只矫正横向。失败模式相对**安全**——结果可能没改善，但不会乱矫正
- **杯子 / 雕塑 / 曲面物体**（基本没直线）：LSD 几乎找不到线段，算法返回原图。**最安全**的失败
- **树林 / 草丛 / 人群**（多直线但弱平行）：**最危险**——LSD 找出大量近似竖直的短段，RANSAC 找到某个能勉强成立的 VP，算法**自信地施加一个方向可能完全错误的矫正**。这种场景下"自动"模式可能让画面变得更差

当前不再对每个 warp 结果重新跑 LSD，而是跟踪同一批源线段经过候选旋转后的方向；改善少于 0.25° 时自动选择“无需调整”。这避免了重采样和重新检测带来的噪声，也把 5984 当前的不确定结果安全地留给用户选择。

### Gravity 与视觉估计

iPhone 拍照时陀螺仪和加速度计是开着的——**HEIC 的 Apple Maker Notes 区里保存了拍摄瞬间的重力向量**（手机相对地面的姿态）。这意味着：
...
|---|---|---|
| 建筑 | ✅ 可工作 | ✅ 更准 |
| 金字塔 / 杯子 / 雕塑 | ❌ 无解 | ✅ 直接给出真实倾斜 |
| 树林 / 无明显结构 | ⚠️ 自信乱矫正 | ✅ 不依赖线段 |
| 镜头畸变 | 不补 | 不补 |
| 截图 / 非相机来源（无 EXIF）| 能跑 | 没数据 → fallback 到 LSD |

GeoCalib spike 对 9 张样本做了独立交叉验证：6 张与 Apple gravity 相差不超过 1.7°，5984 为 1.64°；5792 相差 11.95°，再次验证 norm gate；但 5587 和 4501 即使 norm 正常仍分别冲突 5.56° 和 9.42°。因此下一步不是“gravity 覆盖视觉 VP”，而是让 Apple、视觉 VP、GeoCalib 各自产生带置信度的提案，冲突时默认不动。详见 `spike_geocalib/FINDINGS.md`。

### 已知的小坑（写在这里供未来回头看）

- **EXIF Orientation 仍只完整覆盖 Orientation=1**：当前 Apple 样本都是 1；其他来源需要补 pixel transpose、gravity 坐标变换和保存时清理 orientation tag 的完整测试。
- **纯旋转只保证“校正方式物理合法”**：错误但小角度的 VP 同样能生成合法 rotation，所以 physics validation 不能代替 estimator confidence。
- **大角度可能是拍摄意图**：5362 的 Apple 与 GeoCalib gravity 几乎完全一致，但自动扶正 40° 仰拍仍然是错误产品决定；大角度需要单独的意图/裁剪损失 gate。

## TODO / 后续

- [x] **源线段前后方向度量 + 无需调整候选**
- [x] **解析 Apple Maker Notes gravity + norm gate**
- [x] **从 EXIF 读取 35mm 等效焦距并统一前后端 K**
- [x] **线段长度自适应重试、both 严格模式、候选共享检测**
