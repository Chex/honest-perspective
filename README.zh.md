...
![街景原图自动完成竖向透视矫正的对比演示](docs/assets/demo.gif)

一个用于矫正照片透视畸变的 prototype。和 Snapseed 的"透视"功能定位类似，但有三条刻意"诚实"的约束：

- **只做裁剪、不做填充**——warp 结果只保留原图有效覆盖范围。几何变换会正常重采样，但不会用黑边、内容感知合成或 AI 生成的"假背景"填补缺失区域。
- **错误的矫正不如不矫正**——自动模式把"无需调整"当作一等候选，证据不够充分时就选它。
- **只重新摆正相机，不把画面当橡皮布拉扯**——手动模式只允许物理合法的相机旋转、同画幅裁剪和统一缩放，不会为了让画面显得更正而引入任意剪切或非均匀拉伸。
...
支持 JPG / PNG / **HEIC**（iPhone "高效率"格式，服务端通过 `pillow-heif` 解码）。浏览器自己解不了的格式，预览会 fallback 到服务端渲染（`POST /preview`）；保存时 warp 的仍是全分辨率原图。
...
- **保存**：桌面端下载 JPG。iOS 在 HTTPS 下可使用 Web Share API 并选择"存储到照片"；上面普通 HTTP 的局域网地址会退化成下载，Safari 通常会把文件存入"文件"。本地后端会保留 EXIF（拍摄时间、相机、GPS、镜头）和 ICC profile；托管 demo 的限制见上文
- 预览图在浏览器里 downscale 到 ~`max(viewport) × DPR`（封顶 2500px），保证大图 matrix3d 60fps；本地后端保存使用原图全分辨率，托管 demo 导出仍受 GPU 纹理上限影响
...
前端 `webapp/geometry.js`、后端 `geometry.py` 和供未来原生 App 使用的 Swift Package `Sources/HonestGeometry` 遵守同一份几何合同。三端使用由 Python reference 生成并提交到仓库的同一组测试向量（`tests/generate_geometry_fixtures.py` → `tests/HonestGeometryTests/Fixtures/geometry_contract.json`），逐项比较 matrix、crop 和角度。保存接口直接发送 correction state，旧 corners API 只作为兼容路径保留。
...
```bash
python -m unittest discover -s tests -v   # Python + 浏览器合同测试（需要 Node.js）
swift test                                # Swift 几何 Package，共用同一组 fixtures
```

有意修改几何合同时，请运行 `python tests/generate_geometry_fixtures.py` 重新生成共享 fixtures；如果代码已变但 fixtures 未更新，测试会失败。
